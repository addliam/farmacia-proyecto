...
const BatchSearch = ({placeholder, productsNameList, triggerOnBlankField, inputSearchHandler}) => {
  ...
  const ClassName1 = `py-2 text-white text-sm px-2 border-b-[1px] border-gray bg-[#5A90DB]`
  const ClassName2 = `py-2 text-sm px-2 border-b-[1px] border-gray`
...
//   }
  ...
  return (
    <div className='search flex flex-row w-[340px] z-20 h-[38px] '>
        <div>
            <input autoComplete='off' ref={inputRef} onKeyDown={_handleKeyDown} value={value} onChange={handleInputChange} className='w-fit text-[14px] h-[40px] bg-[#E3EBF3] rounded-l px-3  focus:outline-2 focus:outline-[#d2d7dd] ' type="text" name="tip" id="tip" placeholder={placeholder} />
            {
                showListSuggestions?(
                <div className='bg-white rounded-b-md '>
                    <ul className='max-h-[480px] overflow-x-auto'>
                        {
        ...
        <div>
            <button type='submit' onClick={()=>console.log("button pressed")} className='px-4 py-2 rounded-r bg-[#E3EBF3] hover:bg-[#BCC2C8] '>
